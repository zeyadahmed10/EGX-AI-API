...
public class KafkaScrapedNewsProducerConfig {
    ...
    @Bean
    public ProducerFactory<String, ScrapedNews> producerNewsFactory(){
        return new DefaultKafkaProducerFactory<>(produceConfigs());
    }

    @Bean
    public KafkaTemplate<String, ScrapedNews> kafkaNewsTemplate(){
        return new KafkaTemplate<>(producerNewsFactory());
    }
}
